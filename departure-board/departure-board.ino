...
const char* hostname   = "www.devinmalanaphy.com";
const char* path = "/wp-content/uploads/2026/01/temp.bmp";
...
void addDateTimeToPageBuffer()
{
  ...
  do
  {
    ...
    // use a copy of values that might change, don't read e.g. from analog or pins in the loop!
    display.fillScreen(GxEPD_WHITE); // set the background to white (fill the buffer with value for white)
    display.setCursor(10, 800 - 20); // set the postition to start printing text
    display.print(timeTextBuffer); // print some text
    // end of part executed multiple times
  }
  // tell the graphics class to transfer the buffer content (page) to the controller buffer
  // the graphics class will command the controller to refresh to the screen when the last page has been transferred
